...
- 微件保留项目图标、项目名、提供方路径、项目路径和 Git 分支，并支持按这些内容搜索；Git 分支追加在现有最后一行，不改变项目行布局。
...
- Project icons, names, provider paths, project paths, and Git branches remain visible and searchable. The Git branch is appended to the existing final line without changing the project-row layout.
